...
class RL_Trainer(object):
    def __init__(self, params):
        ...
        # self.env.seed(seed) This function is already DEPRECATED, must set when reset env
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:  # the first iteration -> used the provided data
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for path_step in range(len(paths)):
            paths[path_step]["action"] = expert_policy.get_action(
                paths[path_step]["observation"]
            )
        return paths
