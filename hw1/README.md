...
First, we train the agent using the expert data.
```
...
Then used the inference.py to evaluate the model by running this command. However, you must change the path and environment name in the script before running.
```
python inference.py
```
Command for Question 1.3:

Similar to the Question 1.2, we first train model on different value of training step, and then evaluate using the same command.
```
python cs285/scripts/run_hw1.py \
	--expert_policy_file cs285/policies/experts/Ant.pkl \
	--env_name Ant-v2 --exp_name bc_ant_iter_{step} -- num_agent_train_steps_per_iter {step} \
	--n_iter 1 --expert_data cs285/expert_data/expert_data_Ant-v2.pkl
	--video_log_freq -1
```

The result can be seen in visualization.ipynb file.
